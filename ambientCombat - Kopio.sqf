/*
				***		ARMA3Alpha AMBIENT COMBAT SCRIPT v1.95 - by SPUn / lostvar	***
		...
		Calling the script:
				...
				custom: 	nul = [min range, max range, min delay, max delay, groups, side ratios, center unit, AI skills, 
								communication, dissapear distance, custom init, patrol type, MP] execVM "ambientCombat.sqf";
...
if(isNil("LV_fullLandVehicle"))then{LV_fullLandVehicle = compile preprocessFile "LV_functions\LV_fnc_fullLandVehicle.sqf";};
if(isNil("LV_fullAirVehicle"))then{LV_fullAirVehicle = compile preprocessFile "LV_functions\LV_fnc_fullAirVehicle.sqf";};
if(isNil("LV_fullWaterVehicle"))then{LV_fullWaterVehicle = compile preprocessFile "LV_functions\LV_fnc_fullWaterVehicle.sqf";};
if(isNil("LV_menGroup"))then{LV_menGroup = compile preprocessFile "LV_functions\LV_fnc_menGroup.sqf";};
if(isNil("LV_diveGroup"))then{LV_diveGroup = compile preprocessFile "LV_functions\LV_fnc_diveGroup.sqf";};
...
if(_communication == 1)then{if(isNil("LV_AIcommunication"))then{LV_AIcommunication = compile preprocessFile "LV_functions\LV_fnc_AIcommunication.sqf";};};
...
if(isNil("LV_FindLandPosition"))then{LV_FindLandPosition = compile preprocessFile "LV_functions\LV_fnc_findLandPosition.sqf";};
	...
	if(count LV_ACS_activeGroups < _groupAmount)then{
		...
		if(((typeName _syncedUnit) == "ARRAY")||(_mp))then{
			...
			while{!_spotValid}do{
				...
				if(surfaceIsWater _spawnPos)then{
					...
					while{count _isFlat == 0}do{ //check if there's land at _maxRange
						...
						_isFlat = _tempPos isflatempty [3,0,0.5,3,0,true];
						_d1 = _d1 + 45;
						if(_d1 == 405)exitWith{};
					};
					if(count _isFlat > 0)then{
						_waterUnitChance = floor(random 5);
						if(_waterUnitChance > 0)then{ //Water units chance 1/5
							_spawnPos = _isFlat;
				...
				{
					if((_x distance _spawnPos) < _minRange)then{_spotValid = false;};
				}forEach _syncedUnit;
			};
		}else{
			...
			if(surfaceIsWater _spawnPos)then{
				...
				while{count _isFlat == 0}do{ //check if there's land at _maxRange
					_tempPos = [(_centerPos select 0) + (sin _d1) * _maxRange, (_centerPos select 1) + (cos _d1) * _maxRange, 0];
					_isFlat = _tempPos isflatempty [1,0,1,0,0,true];
					_d1 = _d1 + 45;
					if(_d1 == 405)exitWith{};
				};
				if(count _isFlat > 0)then{
