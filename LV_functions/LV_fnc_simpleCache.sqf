/*ARMA3Alpha function LV_fnc_simpleCache v0.5 - by SPUn / lostvar

		This script caches fillHouse & militarize scripts.

	nul = [[ID's],[players],distance,keep count,MP] execVM "LV_functions\LV_fnc_simpleCache.sqf";
	...
	nul = [[13,14],[playerUnit1],500,true,false] execVM "LV_functions\LV_fnc_simpleCache.sqf";
	...
	for[{_i=0}, {_i<(count _ids)}, {_i=_i+1}] do{
		...
		if((isNil("LVgroup"+(str (_ids select _i))+"spawned"))&&(_inRange))then{
			...
			if(_script == "militarize")then{ 
				call compile format["nul = %1 execVM 'militarize.sqf';",_params];
			}else{
				call compile format["nul = %1 execVM 'fillHouse.sqf';",_params];
			};
